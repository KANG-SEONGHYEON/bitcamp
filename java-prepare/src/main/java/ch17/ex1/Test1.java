 ...
 * 2) Eclipse의 자바 프로젝트 속성에서 사용하려는
 *    JDBC Driver 파일(.jar)의 경로를 build path에 추가한다.
 * 잠깐!
 * => 자바 프로그램에서 다른 개발자가 만든 라이브러리를 사용하기 위해서는
 *    이렇게 다운로드하고 프로젝트의 빌드 경로에 추가해야 한다.
 * => 이렇게 반복적이고 번거로운 빌드 작업을
 *    단순화 시키는 도구를 "빌드 도구"라 부른다.
 * => 대표적인 제품이 ant, maven, gradle이다.
 * => 빌드 도구를 사용하면 프로그램에서 사용하는 라이브러리(의존 라이브러리)를
 *    자동으로 다운로드 받고
 *    이클립스 프로젝트의 빌드 경로에 자동으로 추가할 수 있다.
 * => Gradle 빌드 도구 사용 예)
 *    - build.gradle 파일의 dependencies{} 블록에 의존 라이브러리 파일 정보를 등록하라!
 *    - 의존 라이브러리 정보는 mvnrepository.com에서 검색해보라!
