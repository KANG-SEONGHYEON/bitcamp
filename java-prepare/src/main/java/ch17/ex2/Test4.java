...
public class Test4 {
	...
	public static void main(String[] args) throws Exception {
		...
		ResultSet rs = stmt.executeQuery(
				"select * from jdbc_test2");
		
		/* ResultSet 도구를 사용하여 서버에 준비 된 select 데이터를 한 개씩 가져오기
		 * => next()를 호출하면 서버에서 데이터를 한 개(레코드라 부른다) 가져온다.
		 *    가져왔으면 true, 더 이상 가져올 레코드가 없으면 false를 리턴한다.
		 */
		while (rs.next()) {
		...
		/* Statement와 마찬가지로 ResultSet 객체는 Statement로 부터
