...
public class Test3 {
	public static void main(String[] args) {
		...
		// 사용자로부터 키보드로 값을 입력 받아 계산하는 프로그램
		Console console = System.console();
		
		// 콘솔을 사용할 수 있는지 여부를 검사한 후 사용하자!
		// => 이전 Test1 예제보다 훨씬 더 예외 상황을 부드럽게 처리한다.
		if (console == null) {
			System.err.println("이클립스가 콘솔 입력을 지원하지 않습니다.");
			System.exit(1);
		}
		...
		while (true) {
			try {
				...
				/* 예외 상황이 발생할 경우 어떻게 처리할 것인지에 대한 코드를 둔다.
				 *
				 * => 이 예제에서는 오류가 발생했을 때
				 *    간단히 안내를 출력하고 프로그램 멈춤없이 계속 실행한다.
				 * 
		...
		/* 예외가 발생할 가능성이 있는 코드를 try...catch... 블록에 묶어두면
		 * 다음 두 가지의 이익을 얻을 수 있다.
		 * 
		 * 1) 예외가 발생하더라도 프로그램을 멈추지 않고 계속 실행할 수 있다.
