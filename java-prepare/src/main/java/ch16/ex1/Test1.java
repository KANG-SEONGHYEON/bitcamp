/* 멀티 태스킹
 * - 한번에 여러 개의 프로그램을 동시에 실행하는 것
 * - 실제는 cpu가 여러 개의 프로그램을 돌아가면서 
 *   일정 시간 분량으로 명령어를 처리한다.
 * - cpu의 처리 속도가 빠르기 때문에 잠깐씩 돌아가면서
 *   각각의 프로그램 명령을 처리해도
 *   사용자에게는 동시에 실행되는 것 처럼 보인다.
 * - 멀티 태스킹을 수행하려면 실행할 프로세스의 순서를 관리해야한다.
...
public class Test1 {
	...
	public static void main(String[] args) {
		// 이 순간 이 코드를 실행하는 스레드 알아내기
		Thread t1 = Thread.currentThread();
		System.out.println(t1.getName());
		// => main()을 호출하는 스레드는 "main"이라는 이름을 가진 스레드이다.
