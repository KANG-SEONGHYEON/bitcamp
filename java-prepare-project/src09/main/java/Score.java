 ...
 * 
 * 클래스 문법은 이렇게 '사용자 데이터 타입을 정의'하는 것과 
 * '메서드를 분류'하는 것을 동시에 하는 경우가 많다.
 */
public class Score {
	
	/* 메모리 설계도
	 * new 명령을 사용하여 메모리를 만들 때
	 * 어떤 메모리를 준비해야 하는 지 정의하는 명령!
	 * 즉, 다음 명령어는 new Score()라는 작업을 수행할 때 실행된다.
	 */
	
	String name; // => 문자열을 저장할 name이라는 메모리를 만들라!
	int[] subjects = new int[3]; // => 국,영,수 점수를 저장할 메모리를 만들라!
	int sum; // => 합계를 저장할 메모리를 만들라!
	float aver; // => 평균을 저장할 메모리를 만들라!
	
	/* 메서드 분류
	 * Score 메모리의 값을 다루는 메서드는
	 * Score 클래스에 두는 것이 더 유지보수에 도움이 된다.
	 */
